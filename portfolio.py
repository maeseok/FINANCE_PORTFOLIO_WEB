...
def buy_save(item, buyprice, buynumber):
    try:
        ...
        buyreturn = item+"\n"+buyprice+"\n"+buynumber
        buypath= "/nomad_coders/python_scrapper/CODE/DB/BUY.txt"
        file = open(buypath, 'a')
...
def sell_save(item, sellprice, sellnumber):
    try:
        sellreturn = item+"\n"+sellprice+"\n"+sellnumber
        sellpath= "/nomad_coders/python_scrapper/CODE/DB/SELL.txt"
...
def stock_item_save(item, sellnumber):
    try:
        stockpath = "/nomad_coders/python_scrapper/CODE/DB/STOCK_ITEM/"+item+".txt"
...
def portfolio_initialize(stock_item):
    try:
        ...
        path="/nomad_coders/python_scrapper/CODE/DB/BUY.txt"
        file = open(path, 'w')
        path="/nomad_coders/python_scrapper/CODE/DB/PROFIT.txt"
        file = open(path, 'w')
        path="/nomad_coders/python_scrapper/CODE/DB/SELL.txt"
        file = open(path, 'w')   
        
        for j in range(0,len(item)):
            itempath = "/nomad_coders/python_scrapper/CODE/DB/STOCK_ITEM/"+item[j]+".txt"
